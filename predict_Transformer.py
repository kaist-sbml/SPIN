...
from model_Transformer import TransformerRegressor, TARGET_COLS

DYN_EXTRA = ["rpm", "Feed", "delta_t"]
ALL_DYN = TARGET_COLS + DYN_EXTRA

CKPT_PATH = Path("./Models/ckpt_Transformer.pt")
...
def main(args):
    ...
    next_timepoint = last_time + float(x_last[-1, ALL_DYN.index("delta_t")])
    ...
    # Case 1: BDO increase predicted -> no RPM search
    ...
    # Case 2: BDO decrease predicted
    else:
        # Low CC and no feed -> add feeding first
        if (curr_cc < 20.0) and (curr_feed == 0.0):
            ...
            if pred_bdo_feed >= curr_bdo:
                ...
                print(f"Predicted BDO at {next_timepoint:.1f}h (with 50 g/L Feeding, RPM={best_rpm:.0f}): {best_bdo:.3f}")
        ...
        # CC >= 20 or feed already > 0 -> direct RPM search
        else:
            print("BDO decrease predicted, RPM recommendation performed.")
            best_rpm, best_bdo = search_best_rpm(model, x_last, curr_rpm, device)
            print(f"Predicted BDO at {next_timepoint:.1f}h (RPM={best_rpm:.0f}): {best_bdo:.3f}")
